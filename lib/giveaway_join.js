...
function GiveawayJoin({ storage, giveawayId, memberId }) {
  try {
    ...
    // Sprawdź limit maxParticipants jeśli jest ustawiony (null oznacza brak limitu)
    if (
      giveaway.maxParticipants !== null &&
      giveaway.participantCount >= giveaway.maxParticipants
    ) {
      // Limit osiągnięty, nie dodajemy, zwracamy false
      return false;
    }

    // Dodaj tylko jeśli użytkownik nie jest już na liście
    if (!giveaway.participants.includes(memberId)) {
      giveaway.participants.push(memberId);
    }

    // Aktualizuj participantCount zawsze
